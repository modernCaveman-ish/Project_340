...
unsigned int lookUpUserfunc(userfunc **userFuncs, char *n){
	unsigned int i=0;
	unsigned int flag = 0;

	for(i=0; i<totalUserFuncs; i++){
		if(strcmp(userFuncs[i]->id, n)==0){
			flag=1;
		}
	}
	return flag;
...
unsigned userfuncs_newfunc(SymbolTableEntry*sym){
 ...
 if(userFuncs==NULL){
		...
		if(lookUpUserfunc(userFuncs, sym->name) ==1){
...
void generate_relational (enum vmopcode op,struct quad *quad) {
	...
	t.opcode = op;
	make_operand(quad->arg1, &t.arg1);
	if(quad->result)make_operand(quad->result, &t.arg2);
